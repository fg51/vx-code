...
fn generate_logfile_config(flags: &HashMap<String, Option<String>>) -> LogfileConfig {
    ...
    let log_dir_flag_option =
        extract_env_or_flag(&flags, &log_dir_env_flag).map(PathBuf::from);

    let log_file_flag_option =
        extract_env_or_flag(&flags, &log_file_env_flag).map(PathBuf::from);
