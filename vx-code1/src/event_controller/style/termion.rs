...
use super::{
    RGBColor, Style, StyleID, StyleRange, Styles, LINE_SECTION_STYLE_ID,
    SELECTION_STYLE_ID, STYLE_LEN,
};
        ...
        if styles.is_empty() {
            ...
            for style in styles {
                res.push_str(&format!(
                    ...
                    unsafe {
                        input.get_unchecked(style.start as usize..style.end as usize)
                    },
                    BG_RESET.as_str(),
                    FG_RESET.as_str(),
                ));
        ...
        while idx < style_len {
            ...
            if styles[idx].start == styles[idx + 1].start {
                ...
                if styles[idx + 1].style.foreground.is_none() {
                    styles[idx + 1].style.foreground =
                        style_after.style.foreground.clone();
                }

                if styles[idx + 1].style.background.is_none() {
                    styles[idx + 1].style.background =
                        style_after.style.background.clone();
                }
